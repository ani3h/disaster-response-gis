...
"""
PostGIS queries are disabled in this project.
These stub functions exist only so imports work safely.
"""
...
def check_point_in_disaster_zone(lat, lon):
    # Always return no danger since no DB
    return {"in_danger": False}


def find_nearest_facilities(*args, **kwargs):
    return []


def get_roads_in_bbox(*args, **kwargs):
    return {"type": "FeatureCollection", "features": []}


def create_buffer_zone(*args, **kwargs):
    return None


def calculate_affected_population(*args, **kwargs):
    return 0
